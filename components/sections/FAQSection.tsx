...
export default function FAQSection({ faqs=[], title, dark=true, groups, decor }:{ faqs?:FAQItem[]; title?:string; dark?:boolean; groups?:FAQGroup[]; decor?:boolean }) {
  ...
  const decorRings = decor ? [
    { top:"-6%", left:"-4%", size:280, o:0.16 },
    { top:"8%", right:"-6%", size:360, o:0.13 },
    { top:"38%", left:"-8%", size:220, o:0.15 },
    { top:"52%", right:"-5%", size:300, o:0.11 },
    { top:"72%", left:"-6%", size:260, o:0.14 },
    { top:"88%", right:"-7%", size:320, o:0.12 },
  ] : [];
  return (
    <section style={{ background:bg,padding:"60px 32px",position:"relative",overflow:"hidden" }}>
      <JsonLd data={schema} />
      ...
      {!dark && <div style={{ position:"absolute",inset:0,backgroundImage:"radial-gradient(ellipse 55% 35% at 50% 0%,rgba(212,168,67,0.07) 0%,transparent 70%)",pointerEvents:"none" }} />}
      {decor && (
        <div style={{ position:"absolute",inset:0,pointerEvents:"none",opacity:1 }}>
          {/* faint dot grid */}
          <div style={{ position:"absolute",inset:0,backgroundImage:"radial-gradient(rgba(212,168,67,0.10) 1px,transparent 1px)",backgroundSize:"34px 34px",opacity:0.5 }} />
          {/* six rings, one per FAQ area */}
          {decorRings.map((r,i)=>(
            <div key={i} style={{ position:"absolute",top:r.top,left:r.left,right:r.right,width:r.size,height:r.size,borderRadius:"50%",border:"1px solid rgba(212,168,67,0.5)",opacity:r.o }} />
          ))}
        </div>
      )}
