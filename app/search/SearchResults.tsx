...
export default function SearchResults({
  ...
  const sm = (mode || undefined) as SearchMode | undefined;
  const st = (type || undefined) as SearchType | undefined;
  const sb = budget || undefined;
  const sq = q || undefined;

  const results = filterListings({ q: sq, mode: sm, type: st, budget: sb });
  const summary = searchSummary({ q: sq, mode: sm, type: st, budget: sb });
  ...
  const modeTabs = (onNavy: boolean) =>
    MODES.map((m) => {
      ...
      const href = paramsToSearchPath({ q: sq, mode: m.value, type: st, budget: sb });
      return (
                  ...
                  <Link href={property.link} className="hover-lift" style={{ display: "flex", flexDirection: "column", flex: 1, minWidth: 0, background: "#fff", border: "1px solid rgba(212,168,67,0.18)", borderRadius: 16, overflow: "hidden", textDecoration: "none" }}>
                    ...
                    <div style={{ padding: 20, flex: 1, display: "flex", flexDirection: "column", height: "100%" }}>
                      ...
                      <p style={{ fontFamily: "var(--t-body)", fontSize: 13, color: "var(--slate)", marginBottom: 4 }}>{property.location}</p>
                       ...
                       <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderTop: "1px solid rgba(212,168,67,0.2)", paddingTop: 12, marginTop: "auto", flexShrink: 0, minHeight: 73, height: 73 }}>
                        <div>
                          ...
                          <p style={{ fontFamily: "var(--t-body)", fontSize: 11, fontWeight: 600, color: "var(--gold-ink)", margin: "2px 0 0", lineHeight: 1.2 }}>{property.config || "\u00A0"}</p>
                        </div>
